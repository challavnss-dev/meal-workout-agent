...
"""
    ...
    def create_plan(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        meal_result = self.meal_agent(state, workout_result)
        constraint_result = self.constraint_agent(
            state, workout_result, meal_result
        )
        final_plan = self.coordinator_agent(
            state, workout_result, meal_result, constraint_result
        )
...
foods_to_avoid = st.sidebar.text_input("Foods to Avoid", "")
...
workout_options = [
    "Strength Training", "Cardio", "Full Body", "Mobility", "Rest"
]
...
for day in days:
    workout_schedule[day] = st.sidebar.selectbox(
        day, workout_options, key="workout_" + day
    )
...
if st.button("🚀 GENERATE COORDINATED PLAN", use_container_width=True):

    with st.spinner("AI agents are coordinating your plan..."):
        ...
        if pantry_image is not None:
            ...
            state["vision_detected_ingredients"] = vision_result.get(
                "ingredients", []
            )
...
if st.session_state.vision_result:
    ...
    ingredients = st.session_state.vision_result.get("ingredients", [])
    ...
    notes = st.session_state.vision_result.get("confidence_notes")
...
if st.session_state.plan_result:
    ...
    for day in days:
        ...
        with st.expander("📌 " + day, expanded=True):
            ...
            if isinstance(day_data, dict):
                st.write("🏋️ **Workout:**", day_data.get("workout", "Not specified"))
                st.write("🍳 **Breakfast:**", day_data.get("breakfast", "Not specified"))
                st.write("🥗 **Lunch:**", day_data.get("lunch", "Not specified"))
                st.write("🍎 **Snack:**", day_data.get("snack", "Not specified"))
                st.write("🍲 **Dinner:**", day_data.get("dinner", "Not specified"))
                st.write("💡 **Reason:**", day_data.get("reason", "Not specified"))
    ...
    with col1:
        st.metric(
            "Estimated Budget",
            "₹" + str(plan.get("estimated_weekly_budget", 0))
        )

    with col2:
        st.metric(
            "Constraint Status",
            str(plan.get("constraint_status", "Not available"))
        )

    with col3:
        st.metric("Agents", len(result.get("agents_used", [])))
    ...
    for index, item in enumerate(grocery_list):
        st.checkbox(str(item), key=f"grocery_{index}")
...
agents_info = [
    ("🥗 Meal Agent", "Creates coordinated meals."),
    ("🏋️ Workout Agent", "Creates workout plans."),
    ("📷 Pantry Vision Agent", "Reads ingredients from pantry images."),
    ("🔍 Constraint Agent", "Checks budget and constraints."),
    ("🧠 Coordinator Agent", "Combines all agent outputs."),
    ("👀 Monitoring Agent", "Detects changes."),
    ("🔄 Replanning Agent", "Updates the plan after changes.")
]

for agent_name, description in agents_info:
    st.info(f"**{agent_name}** — {description}")
...
if st.button("🔍 DETECT CHANGE & REPLAN", use_container_width=True):
    ...
    if st.session_state.current_state is None:
        ...
        with st.spinner("Monitoring and replanning..."):
            ...
            monitor_result = agents.monitoring_agent(old_state, new_state)
            st.session_state.monitor_result = monitor_result

            if monitor_result.get("replan_required", False):

                old_plan = {}

                if st.session_state.plan_result:
                    old_plan = st.session_state.plan_result.get("plan", {})

                replan_result = agents.replanning_agent(
                    old_state, new_state, old_plan
                )
...
if st.session_state.monitor_result:
    ...
    if monitor.get("change_detected", False):
        st.warning("Change detected.")
    else:
        st.success("No important change detected.")
    ...
    if monitor.get("affected_days"):
        st.write(
            "**Affected days:** "
            + ", ".join(str(x) for x in monitor["affected_days"])
        )

    if monitor.get("affected_components"):
        st.write(
            "**Affected components:** "
            + ", ".join(str(x) for x in monitor["affected_components"])
        )
...
if st.session_state.replan_result:
    ...
    updated_plan = replan.get("updated_plan", {})

    for day in days:

        day_data = updated_plan.get(day, {})
    ...
    if replan.get("new_grocery_list"):
        ...
        for index, item in enumerate(replan["new_grocery_list"]):
            st.checkbox(str(item), key=f"updated_grocery_{index}")
...
st.write("Observe → Analyze → Check → Decide → Act → Monitor → Replan")

st.divider()

st.caption("Built with Streamlit + Groq + Multi-Agent AI")
